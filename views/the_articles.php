...
<section id="revue">
    ...
    <div id="auteurs">
        ...
        <?php $max = sizeof($include['datas']['auteurs']); ?>
            <?php for($i=0; $i<$max; $i++): ?>
                <li><a href='index.php?page=article&co_revue=<?= $include["datas"]["revue"]->getco_revue() ?>&personne_id=<?= $include['datas']['auteur'][$i]['personne_id'] ?>'><?= $auteur->getPer_titre() ?><?= $auteur->getPer_nom() ?></a></li>
            <?php endfor ?>
        <?php endif ?>
        </ul>
    </div>
