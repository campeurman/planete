    ...
    <div id="articles" class="sm-col-12">
            ...
            <?php else: ?>
                <h2>Articles</h2>
                    ...
                    <?php for($i=0; $i<$max; $i++): ?>
                        <li><a href='index.php?page=article&co_revue=<?= $revue->getco_revue() ?>&personne_id=<?= $auteur->getPersonne_id() ?>&rubrique_id=<?= $rubrique->getRubrique_id() ?>&num_article=<?= $articles[$i]["num_article"] ?>'><?= $articles[$i]["art_titre"] ?></a></li>
                    <?php endfor ?>
                </ul>
            <?php endif ?>
        </div>
